...
template_dir = f"{os.getcwd()}/templates/MCQA_RL_likelihood"  # seems jinja wants the absolute path
...
def mcqa_reward_function(
    prompts, completions, choices, correct_answer_letter, **kwargs
):
    """Compute rewards for MCQA responses"""
    rewards = []

    for completion, choice_list, correct_answer in zip(
        completions, choices, correct_answer_letter
    ):
...
@hydra.main(
    config_path="config", config_name="MCQA-Likelihood_RL.yaml", version_base="1.1"
)
